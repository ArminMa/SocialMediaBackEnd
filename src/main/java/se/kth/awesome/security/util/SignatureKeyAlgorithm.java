package se.kth.awesome.security.util;

import com.springcryptoutils.core.signature.SignatureException;
...
public class SignatureKeyAlgorithm {
	...
	public static void enableBouncyCastle(){
		...
		for(Provider provider : providers) {
			if (BouncyCastleProvider.class.isInstance(provider)) {
				bcLoaded.set(true);
				return;
			}
		}
	...
	public static boolean isBouncyCastleProvider(){
		return bcLoaded.get();
	}
	...
	public static enum Algo {
		...
		Algo(String value, String description, String familyName, String jcaName, boolean jdkStandard) {
			this.value = value;
		...
		/**
		 * Looks up and returns the corresponding {@code SignatureKeyAlgorithm} enum instance based on a
		 * case-<em>insensitive</em> name comparison.
		 *
		 * @param value The case-insensitive name of the {@code SignatureKeyAlgorithm} instance to return
		 * @return the corresponding {@code SignatureKeyAlgorithm} enum instance based on a
		 * case-<em>insensitive</em> name comparison.
		 * @throws SignatureException if the specified value does not match any {@code SignatureKeyAlgorithm}
		 *                            name.
		 */
		public static Algo forName(String value) throws SignatureException {
			for (Algo alg : values()) {
